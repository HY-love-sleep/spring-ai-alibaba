...
public class DifyDSLAdapter extends AbstractDSLAdapter {
	...
	public Workflow mapToWorkflow(Map<String, Object> data) {
        ...
        List<Variable> startVars = graph.getNodes().stream()
                .filter(n -> NodeType.START.value().equals(n.getType()))
                .flatMap(n -> ((List<Map<String,Object>>)n.getData().get("variables")).stream())
                .map(m -> mapper.convertValue(m, Variable.class))
                .toList();
